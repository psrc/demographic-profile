...
# 2020 Decennial Data ------------------------------------
# Pull Data
p9_raw <- get_decennial_recs(geography = 'county',
                             sumfile = 'sf1',
...
p9$name <- gsub(" County", "", p9$name)
p9$name <- gsub(", Washington", "", p9$name)

# Export raw data
file_name_raw <- paste(raw_export, collapse = "_")
file_name_raw <- paste(file_name_raw, ".csv", sep = "")
write.csv(p9, file = file.path(export_path, file_name_raw), row.names = FALSE)
